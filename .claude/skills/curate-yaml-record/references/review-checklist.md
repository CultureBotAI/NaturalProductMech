...
| Bioactivity summary | Every `bioactivity_summary` value satisfies the single backing rule in `PLAN.md` §3.5: a `bioactivities` item, a `molecular_targets` item whose target implies the class, a `related_records` link to a sibling corpus carrying the mechanism, or a cited source assertion. | No summary value floats, and the record says which of the four backs it. |
