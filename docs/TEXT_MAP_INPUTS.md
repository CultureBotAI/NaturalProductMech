...
The old `just embed`, `just embed-canary` and `just embed-map` generation paths
are retired. They exit with this guide's location before loading a model or
writing artifacts, including when a custom model is supplied. `just embed-dry`
only previews the historical document builder. Use the locked runtime steps
above to refresh `data/text_map`; retained historical arrays and chemical
specialty views are not regenerated by these commands. Historical artifact
checks guard their structure and links; current-text freshness belongs to the
strict common-map publication gate.
