"""Write-time validation: dump a NaturalProductRecord to YAML *only if* it passes
...
DEFAULT_SCHEMA_PATH = Path(__file__).resolve().parents[1] / "schema" / "naturalproductmech.yaml"
DEFAULT_TARGET_CLASS = "NaturalProductRecord"
...
class ValidationFailedError(Exception):
    """Raised when a NaturalProductRecord fails closed-schema validation before write."""
...
def emit_natural_product_yaml(doc: dict[str, Any], yaml_kwargs: dict[str, Any] | None = None) -> str:
    """Serialise ``doc`` exactly as :func:`write_validated_natural_product` writes it."""
    return yaml.safe_dump(doc, **{**EMIT_OPTS, **(yaml_kwargs or {})})


def write_validated_natural_product(
    doc: dict[str, Any],
    path: Path,
    *,
    target_class: str = DEFAULT_TARGET_CLASS,
